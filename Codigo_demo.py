...
class LogoClientMock:
    def __init__(self):
        ...
        self.fuerza_minima = 280 + OFFSET      # Fuerza Mínima por defecto (280 kg)
        self.fuerza_minima_off = 270 + OFFSET  # VW2
    ...
    def db_read(self, db_number, start_address, size_bytes):
        ...
        # vw6 (Fuerza actual amplificada para grafica)
            ...
            if self.piston_active:
                ...
                target_peak = (self.fuerza_minima - OFFSET) + 75
                # Curva logarítmica de subida con ruido
                fuerza_calc = target_peak * (1.0 - math.exp(-self.sim_time_in_cycle * 2.0)) + random.uniform(-6, 6)
                if self.sim_time_in_cycle > 3.0: # Simular que se estabiliza
                    fuerza_calc = target_peak + random.uniform(-4, 4)
            else:
                self.sim_time_in_cycle = 0.0
